module Network.JMAP.Core
  ( SessionResourceAccount (..),
    SessionResource (..),
    Capability (..),
    MethodCallArg (..),
    methodCallArgFrom,
...
import Data.Map (Map, findWithDefault, fromList, keys, toList)
...
fieldLabels prefix a =
  let constr_fields = map constrFields . dataTypeConstrs . dataTypeOf $ a
   in map (dropAndLowerFirst prefix) (head constr_fields)
...
fromCapabilityName s = case lookup s capabilityNames of
  Just c -> c
...
data MethodCall = MethodCall
  { methodCallCapability :: Capability,
...
instance Aeson.ToJSON MethodCallArgs where
  toJSON (MethodCallArgs m) =
    ...
    where
      toEntry (key, MethodCallArg val) = key .= val
      toEntry (key, ResultReference call path) =
        T.cons '#' key
          .= Aeson.object
            [ "resultOf" .= methodCallId call,
...
instance Aeson.ToJSON Request where
  toJSON (Request calls) =
    ...
    where
      capabilities = Set.toList $ Set.fromList $ map methodCallCapability calls

data MethodResponse = MethodResponse
  { methodResponseName :: T.Text,
...
instance Aeson.FromJSON MethodResponse where
  parseJSON s =
    ...
    where
      convert (name, body, id) =
        MethodResponse
          { methodResponseName = name,
            methodResponseBody = body,
            methodResponseId = id
          }

data Response = Response
  { responseMethodResponses :: [MethodResponse],
...
data MethodCallError
  = ParseBodyError Aeson.Value
...
methodCallResponse i id response
  ...
  | otherwise =
    let body = methodResponseBody (matched !! i)
     in case Aeson.Types.parse Aeson.parseJSON body of
...
instance Aeson.ToJSON FilterCondition where
  ...
  toJSON (FilterOpAND conds) =
    Aeson.object
      [ "operator" .= ("AND" :: String),
        "conditions" .= Aeson.toJSON conds
      ]
  toJSON (FilterOpOR conds) =
    Aeson.object
      [ "operator" .= ("OR" :: String),
        "conditions" .= Aeson.toJSON conds
      ]
  toJSON (FilterOpNOT conds) =
    Aeson.object
      [ "operator" .= ("NOT" :: String),
        "conditions" .= Aeson.toJSON conds
      ]
...
instance Aeson.ToJSON SortComparator where
  toJSON (SortAscending s) =
    Aeson.object
      [ "property" .= s,
        "isAscending" .= True
      ]
  toJSON (SortDescending s) =
    Aeson.object
      [ "property" .= s,
        "isAscending" .= False
      ]
...
data CommonGetResponseBody a = CommonGetResponseBody
  { getResponseAccountId :: AccountId,
...
data CommonQueryResponseBody a = CommonQueryResponseBody
  { queryResponseAccountId :: AccountId,
...
data CommonChangesResponseBody id = CommonChangesResponseBody
  { changesResponseAccountId :: AccountId,
    changesResponseOldState :: GetState,
    changesResponseNewState :: GetState,
    changesResponseHasMoreChanges :: Bool,
    changesResponseCreated :: [id],
    changesResponseUpdated :: [id],
    changesResponseDestroyed :: [id]
  }
  deriving (Show, Generic)

instance (Aeson.FromJSON id) => Aeson.FromJSON (CommonChangesResponseBody id) where
  parseJSON = Aeson.genericParseJSON $ aesonOptionWithLabelPrefix "changesResponse"
